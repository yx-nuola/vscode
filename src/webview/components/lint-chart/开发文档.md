...
| 大组 | 否 | 推荐选择 `device-id`；不选择时整个文件作为一个大组 |
...
`sourceRowIndex` 用于 Tooltip、错误提示和定位原始 CSV 行，不能因为分组而丢失。
...
function isNewBigGroup(
  ...
  return currentRow.values[deviceColumn] !==
    previousRow.values[deviceColumn];
}
```

### 7.3 现有样例兼容

如果旧数据只有 `WL`、`BL`，没有 `device-id`，可以在适配层生成虚拟字段：
...
```text
WL=121, BL=5 -> device-id="121-5"
```

虚拟字段仅用于兼容旧样例；正式数据优先使用 CSV 中真实的 `device-id`。
...
- CSV 原始行号仍通过 `sourceRowIndex` 保留，并显示在 Tooltip 中。
...
```text
src/webview/components/lint-chart/
├── index.tsx
├── LineChartPage.tsx
├── LineChartToolbar.tsx
...
- `csv-parser.ts`：CSV 文本解析和 BOM 处理。
- `column-inference.ts`：字段类型、显示名和单位推断。
...
8. 空 CSV、只有表头的 CSV、重复表头需要给出明确提示。
