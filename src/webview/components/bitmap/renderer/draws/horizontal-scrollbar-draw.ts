...
export class HorizontalScrollbarDraw extends BaseScrollbarDraw {
  ...
  protected getScrollStateFromSlider(state: ScrollbarRenderState): ScrollState {
    ...
    const scrollX = this.engine
      .getVirtualScrollSync()
      .getScrollXFromSlider(this.slider?.x() ?? 0, state.area.width);
  ...
  protected getScrollStateFromTrackClick(
    ...
    const scrollX = this.engine
      .getVirtualScrollSync()
      .getScrollXFromSlider(sliderCenterX - state.scrollbar.sliderWidth / 2, state.area.width);
