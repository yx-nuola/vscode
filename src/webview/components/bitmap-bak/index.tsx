...
const BitmapVisualizationInner: React.FC = () => {
  ...
  // 准备 MatrixData
  const matrixData = useMemo<MatrixData<CellData> | null>(() => {
    if (!state.data) {
      return null;
    }
    return {
  ...
  // 颜色配置
  const colorRanges = useMemo<ColorRange[]>(() => {
    return state.colorConfig.ranges.map(r => ({
      min: r.min,
      max: r.max,
      color: r.color,
      label: r.label,
    }));
  }, [state.colorConfig.ranges]);

  // 选中格子时的联动
  useEffect(() => {
    if (!state.viewState.selectedCell) return;
    
    gridRef.current?.zoomToCell(
  ...
  const handleCellClick = useCallback((event: { coord: { row: number; col: number }; cell: CellData | null }) => {
    ...
    setSelectedCellData(event.cell);
    setShowPopup(true);
    dispatch({ 
      type: 'SELECT_CELL', 
      payload: { bl: event.coord.row, wl: event.coord.col } 
    });
  }, [dispatch]);
    ...
    <div className="bitmap-visualization">
      ...
      <div className="bitmap-content">
        ...
        >
          ...
          {!isUploading && renderer && matrixData && (
            <BitmapGrid
              ref={gridRef}
              renderer={renderer}
              data={matrixData}
